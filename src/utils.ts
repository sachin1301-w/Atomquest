...
export function getQuarterLabel(date: Date): string {
  ...
  if (month >= 4 && month <= 5) return 'Goal Setting';
  if (month >= 6 && month <= 8) return 'Q1 Check-in';
  if (month >= 9 && month <= 11) return 'Q2 Check-in';
  if (month >= 0 && month <= 1) return 'Q3 Check-in';
  return 'Q4 / Annual';
}

export function getQuarterCode(date: Date): string {
  const month = date.getMonth();
...
export function calculateProgress(goal: GoalItem): number {
  if (goal.uom === 'Zero') {
  ...
  if (goal.uom === 'Timeline') {
    ...
    if (!goal.actualDate) return goal.status === 'Completed' ? 0 : 25;
...
export function validateSheet(goals: GoalItem[]): string[] {
  const errors: string[] = [];
...
export function summarizeProgress(goals: GoalItem[]): { completed: number; onTrack: number; notStarted: number; average: number } {
  const counts = goals.reduce(
    (acc, goal) => {
      if (goal.status === 'Completed') acc.completed += 1;
      if (goal.status === 'On Track') acc.onTrack += 1;
      if (goal.status === 'Not Started') acc.notStarted += 1;
...
export function toCsv(rows: Record<string, string | number | undefined>[]): string {
  ...
  const quoted = (value: string | number | undefined) => {
...
export function statusTone(status: GoalStatus | SheetStatus): string {
  switch (status) {
    case 'Completed':
    case 'Approved':
    case 'Locked':
...
export function directionLabel(direction: PerformanceDirection): string {
  return direction === 'Max' ? 'Lower is better' : 'Higher is better';
}

export function uomLabel(uom: UomType): string {
  return uom;
}